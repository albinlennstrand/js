...
Webflow.push(function() {
	...
	$(window).ready(() => {
    if (window.$memberstackDom !== undefined) {
							...
							if (thisId in interactData) {
								if (interactData[thisId] > 0) {
									if (interactData[thisId] == 1) {
										...
										$("#" + trigger).find(".qdigit").addClass("active");
										$("#" + trigger).find(".qdigit").text(interactData[thisId]);
										$("#" + trigger).find(".firstplay").removeClass("active");
										$("#" + trigger).find(".sndplay").removeClass("active");
									}
								}
							}
						}
			...
			if (player in interactData) {
				if (interactData[player] > 0) {
					if (interactData[player] == 1) {
						$("#" + trigger).find(".firstplay").addClass("active");
					} else if (interactData[player] == 2) {
						$("#" + trigger).find(".firstplay").addClass("active");
						$("#" + trigger).find(".sndplay").addClass("active");
					} else if (interactData[player] > 2) {
						$("#" + trigger).find(".playdigit").addClass("active");
						$("#" + trigger).find(".playdigit").text(interactData[player]);
						$("#" + trigger).find(".firstplay").removeClass("active");
						$("#" + trigger).find(".sndplay").removeClass("active");
					}
				}
			...
			if (controls.length) {
				...
				audioSel.onended = () => {
					if (thisMainBtn.hasClass("playing")) {
						...
						var intData = {
                int_body: JSON.stringify(interactData),
                user_id: currentuserid,
                int_target: id,
                cms_id: currentuserid + "_" + id
            };
            
						var hookUrl;
            if (appName == "aviarchive") {
            	hookUrl = "https://hooks.zapier.com/hooks/catch/13972579/2by7wiu/";
            } else if (appName == "avi21") {
            	hookUrl = "https://hooks.zapier.com/hooks/catch/13972579/2bypzqs/";
            }
