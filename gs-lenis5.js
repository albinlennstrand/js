  ...
  var animTimeout;
  let didInit = false, isAnim, currentIndex = -1, allowScroll = true, slidePos, lastSwipeDelta = null, belowSlide = false;
  ...
  function updateSlider(scroll, delta) {
      ...
      if (!allowScroll) {
        if (delta > 0) { //DOWN
          if ((currentIndex + 1) < $(".slide-wrap").find(".bg").length) {
            !isAnim && gotoSection(currentIndex + 1, 1);
          } else {
            if (!isAnim) {
        ...
        if (delta > 0) { //DOWN
          ...
          if (!isAnim && allowScroll && (scroll > (slidePos - ($(window).height() - 50)) && !belowSlide)) {
            if (!didInit) {
              gotoSection(0, 1);
              didInit = true;
            }
          }
          if (!isAnim && allowScroll && (scroll > slidePos) && !belowSlide) {
            allowScroll = false;
            lenis.stop();
            isAnim = true;
            clearTimeout(animTimeout);
          ...
          console.log("lenisscroll2: " + scroll);
          console.log("slidePos: " + slidePos);
          console.log("allowscroll: " + allowScroll);
          console.log("isAnim: " + isAnim);
          console.log("animfirst: " + (slidePos - ($(window).height() - 50)));
          if (!isAnim && allowScroll && (scroll < (slidePos + 50)) && belowSlide) {
            allowScroll = false;
