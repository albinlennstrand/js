  ...
  $(document).ready(() => {
    slidePos = $(".slide-wrap").offset().top;
    $('.page-wrapper').on('scroll mousewheel', function(e){
  ...
  function updateSliderPos(lenisSc, delta) {
      ...
      if (!allowScroll) {
        ...
        if (delta > 0) { //DOWN
          console.log("lenisSc: " + lenisSc);
          console.log("slidePos: " + slidePos);
          if (!isAnimating && allowScroll && (lenisSc > (slidePos - ($(window).height() - 50)) && !belowSlide)) {
            if (!didInit) {
              //gotoSection(0, 1);
              didInit = true;
            }
          }
          if (!isAnimating && allowScroll && (lenisSc > slidePos) && !belowSlide) {
            allowScroll = false;
          ...
          if (!isAnimating && allowScroll && (lenisSc < (slidePos + 50)) && belowSlide) {
            allowScroll = false;
            $("#slide-wrap").addClass("fixed");
            lenis.stop();
            isAnimating = true;
