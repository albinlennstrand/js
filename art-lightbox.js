            ...
            thImg.on('load', function() {
              if (this.complete && this.naturalWidth > 0) {
                ...
                if (detLoad == v.detailImages.length) {
                  $.each(thisDiv.find(".lightbox-thumb"), (z, c) => {
                    var tmpWidth = $(c).width();
                    var tmpHeight = $(c).height();
                    setTimeout(() => {
                      $(c).parent().css("width", tmpWidth);
                      $(c).parent().css("max-width", tmpWidth);
                      $(c).parent().css("height", tmpHeight);
                      $(c).parent().css("max-height", tmpHeight);
                    }, 10);
                  });
                  setTimeout(() => {
                    if (!isVid) {
                      thisDiv.css("width", thisDiv.find(".img-main").find(".lightbox-thumb").first().width());
                      thisDiv.css("max-width", thisDiv.find(".img-main").find(".lightbox-thumb").first().width());
                    } else {
                      thisDiv.css("width", thisDiv.find(".img-link").find(".lightbox-thumb").first().width());
                      thisDiv.css("max-width", thisDiv.find(".img-link").find(".lightbox-thumb").first().width());
                    }
                    thisDiv.slick({
                      infinite: false,
                      arrows: false,
  ...
  function doResize(noanim) {
    if (navArrowType == "halfscreen") {
      if ($(window).width() < 991) {
        if ($(".nav-arrow").hasClass("halfscreen")) {
          $(".nav-arrow").removeClass("halfscreen"); 
          $(".arrow-cursor").addClass("mousehidden");
        }
      } else {
        if (!$(".nav-arrow").hasClass("halfscreen")) {
          $(".nav-arrow").addClass("halfscreen"); 
          $(".arrow-cursor").removeClass("mousehidden");
        }
      }
    }
    if (!isResizing) {
