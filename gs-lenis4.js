    ...
    $('#pagewrap').on('scroll mousewheel', function(e){
      ...
      if (!allowScroll) {
        ...
        if (delta > 0) { //DOWN
          ...
          if (!isAnim && allowScroll && (lenisScroll > (slidePos - ($(window).height() - 50)) && !belowSlide)) {
            if (!didInit) {
              gotoSection(0, 1);
              didInit = true;
            }
          }
          if (!isAnim && allowScroll && (lenisScroll > slidePos) && !belowSlide) {
            allowScroll = false;
            lenis.stop();
            isAnim = true;
            clearTimeout(animTimeout);
          ...
          console.log("isAnim: " + isAnim);
          console.log("animfirst: " + (slidePos - ($(window).height() - 50)));
          if (!isAnim && allowScroll && (lenisScroll < (slidePos + 50)) && belowSlide) {
            allowScroll = false;
